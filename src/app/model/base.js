...
const baseModel = {
  ...
  update: async function (where, data, writeLogs = null) {
    ...
    const fields = [...new Set([...this.$guarded, ...this.$fillable, ...this.$hidden])]; // 过滤不存在的字段
    const dataRow = fields.reduce((result, key) => {
      ...
      if (data.hasOwnProperty(key) && (data[key] || data[key] !== undefined)) {
        ...
        if (castType) {
          ...
          result[key] = data[key] === false || data[key] === null ? data[key] : (data[key] || '').toString();
        }
      }
      // 返回累积的结果对象
      return result;
  ...
  saveDo: async function (data, writeLogs = null) {
    const dbWrite = dbInterface.dbWrite();
    const fields = [...new Set([...this.$guarded, ...this.$fillable, ...this.$hidden])]; // 这是你的键数组
    // 使用reduce函数对fields中的每个键进行处理，以构建一个新的数据对象
    const dataRow = fields.reduce((result, key) => {
      // 检查data对象是否具有当前键，并且该键对应的值不为空或未定义
  ...
  forceDelete: async function (where) {
    ...
    const dbWrite = dbInterface.dbWrite();
    try {
      return await dbWrite(this.$table).where(where).del();
    } catch (error) {
      logInterface.writeError(__filename + ':' + error.toString());
      return false;
    }
